...
{
    public partial class Lissajous_Form : Form
    {
        ...
        public static void RunPythonScript(string args = "", params string[] teps)
        {
            ...
            string path = @"C://Users//hpsin//QT_RS//WindowsFormsApp1//csv_lissa.py";//(因为我没放debug下，所以直接写的绝对路径,替换掉上面的路径了)
        ...
        public static Boolean collinear_3D(Joint hand_start, Joint present_hand)
        {
            // Calculation the area of  
            // triangle. We have skipped  
            // multiplication with 0.5  
            ...
            System.Numerics.Vector3 v1 = System.Numerics.Vector3.Normalize( new System.Numerics.Vector3(hand_start.Proj.X, hand_start.Proj.Y, hand_start.Proj.Z));
            System.Numerics.Vector3 v2 = System.Numerics.Vector3.Normalize(new System.Numerics.Vector3(present_hand.Proj.X,present_hand.Proj.Y,present_hand.Proj.Z));
            //double a = (p1.Y - p2.Y) * (p1.X - p3.X) - (p1.Y - p3.Y) * (p1.X - p2.X);
            double distance = Math.Abs(System.Numerics.Vector3.Distance(v1, v2));
            Console.WriteLine("distance: {0}", distance);
            if (distance < 0.00010)
            {
                Console.WriteLine("distance: {0}", distance);
        ...
        public Lissajous_Form() {
            
            InitializeComponent();
            front_or_side_box.SelectedIndex = 1;
                    ...
                    foreach (var skeleton in _skeletonData.Skeletons)
                    {
                        ...
                        if (side_item == 0 || side_item == 1)
                        {
                            ...
                            if (!calib_hand_start.IsEmpty)
                            {
                                ...
                                e.Graphics.DrawString("Recording.......", new System.Drawing.Font("Arial", 14), new SolidBrush(Color.White), new PointF(0,0));
                            ...
                            if (!calib_hand_start.IsEmpty)
                            {
                                ...
                                //    left_count++;
                                //}
                                if (split_point_datetimes.Count > 100 && collinear_3D(start_hand,hand_joints))
                                {
                                    bool_time_series = false;
                                    calib_hand_end = new PointF(10,0);
                                    calib_hand_start = new PointF();
                                    end_num = frame_count;
                                    MessageBox.Show("Finish collecting, please save!");
                                }
                            }
        ...
        private void button_process_Click(object sender, EventArgs e)
        {
            if (!calib_hand_end.IsEmpty && !calib_hand_end.IsEmpty)
            {
                ...
                //passed_save_dir = "C://Users//hpsin//Desktop";
                ...
                else
                {
                    ...
                    string plane_name = "Lissajous";
                    //string Dir_path = file_image_name.Split('.')[0] + plane_name.Split(' ')[0];
                    string Dir_path = Path.Combine(passed_save_dir, plane_name);
                    ...
                    //final_plot.Save(Path.Combine(Dir_path,"Result.png"));
